...
// BLUETOOTH HEADER
//---------------------------
#define PIN_BLUETOOTH_0 0 //creo que el bluetoth eran los pines 0 y 1.
#define PIN_BLUETOOTH_1 1
String mensaje;
bool tiene_mensajes; //no se si se maneja con un bool o con una función del módulo de bluetooth, BORRAR SI NO HACE FALTA.
...
bool verify_bluetooth()
{
  if(tiene_mensajes) //ver si está vacío el buffer
  {
    mensaje = bt.read(); //o como sea para leer
    if(mensaje == "detener")
    {
        event = BUTTON_1_ACTIVATED; //misma logica para cuando se apreta el botón por primera vez, pasa a vaciando.
        return true;
    }
    if(mensaje == "limpiada")
    {
        event = BUTTON_2_ACTIVATED; //misma logica para cuando se apreta el botón por segunda vez, pasa de vaciando a limpio.
        return true;
    }

    if(mensaje == "getState") //o getHumidity o lo que se nos ocurra
    {
        //para mostrar el estado o el nivel de humedad o lo que se nos ocurra.
        //Se pedía para el TP un comando para informar algo por Serial.print().
        //ME PARECE QUE DEBERÍA SER UN NUEVO ESTADO CON SU EVENTO ASOCIADO CON RECIBIR ESTE COMANDO, y desde state_machine() hacemos el Serial.print().
    }

    return false; //si el mensaje no es ninguno de los 2 comandos que le podemos mandar (o 3), lo ignoramos y seguimos evaluando los otros sensores.
  }

  return false; //si no hay ningún mensaje seguimos evaluando los otros sensores.
}
