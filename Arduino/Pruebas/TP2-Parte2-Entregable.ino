    ...
    case CLEAN:
      switch(event)
      {
        ...
        case GET_STATE:
            ...
            break;
        case GET_HUMIDITY:
            bluetooth_send_humidity(analogRead(moisture_sensor));
            break;
        default:
        break;
      }
...
bool verify_bluetooth()
{
  if(Bluetooth.available())
  {
    ...
    if (bt_receive_msg == BLUETOOTH_MSG_GET_HUMIDITY)
    {
        event = GET_HUMIDITY;
        return true;
    }
...
{ 
  ...
  if(exited == true)
  {
    ...
    {
      bluetooth_send_humidity(moisture);
...
void bluetooth_send_humidity(int value)
{
  // snprintf(bt_send_msg, BLUETOOTH_MSG_SIZE, "%d", value);
